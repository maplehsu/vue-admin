<template>
  <Form :model="formItem" :rules="ruleValidate" :label-width="80" ref="formItem" id="form" enctype="multipart/form-data">
    ...
    <FormItem label="大赛类型" prop="CompetitionTypeID">
      <template>
    ...
    <FormItem label="封面" prop="ImgUrl" class="tooltip-box">
      <template>
        ...
        <Upload ref="upload" :action="url" :format="['jpg','jpeg','png']" :max-size="512000"  :headers="header" :data="params" :before-upload="uploadImage" :on-success="successImage" :on-remove="removeImg">
...
export default {
  ...
  data: function () {
    return {
      ...
      ruleValidate: {
         ...
         Sort:[ { required: true, validator: this.G.validateNumber, trigger: "blur" } ],
         ImgUrl: [
  ...
  mounted: function () {
    this.axios.get(this.api.dictionaryManage, {
      params: {
      ...
      if (res.data.Success === true) {
        res.data.Data.forEach(data => {
          this.select.push({'label': data.DicName, 'value': data.DicID})
        })
      }
    })
